...
<div class="body_table">
	<table>
		...
		$sql = "SELECT beer.img, beer.name, beer.brewery, beer.brew_city, beer.city_state, beer.beer_type, beer.abv, beer.ibu, AVG(user_beer.rating) FROM `beer`,`user_beer` WHERE beer.id=user_beer.beer_id GROUP BY user_beer.beer_id ORDER BY AVG(user_beer.rating) DESC";
			$result = $conn->query($sql);
			if ($result->num_rows > 0) {
			// output data of each row
